...
lowerAndGreater n list =
    let greater = 0
        lower = 0
    in go list greater lower
        where
            go :: [Integer] -> Integer -> Integer -> [Char]
            go list greater lower
                | null list = show n ++ " is greater than " ++ show greater ++ " elements and lower than " ++ show lower ++ " elements"
                | n > head list = go (drop 1 list) (greater + 1) lower
                | n < head list = go (drop 1 list) greater (lower + 1)
                | otherwise = go (drop 1 list) greater lower
